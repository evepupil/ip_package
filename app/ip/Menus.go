...
func LoadMenus(w fyne.Window, app fyne.App) {
	tools_key := []string{"文件(F)", "过滤(E)", "排序(V)", "切换(W)", "模式(M)", "发送(S)", "停止(T)"}
	//"捕获(C)", "分析(A)", "统计(S)", "电话(Y)",
	//"无线(W)", "工具(T)", "帮助(H)",
		...
		},
		{
			func() {
				...
				form := &widget.Form{
					...
					OnSubmit: func() { // optional, handle form submission
						mac_regexp := regexp.MustCompile("/^[A-F0-9]{2}([-:]?[A-F0-9]{2})([-:.]?[A-F0-9]{2})([-:]?[A-F0-9]{2})([-:.]?[A-F0-9]{2})([-:]?[A-F0-9]{2})$")
						if !mac_regexp.MatchString(source_mac_entry.Text) {
							dialog.NewInformation("提示", "源mac: +"+source_mac_entry.Text+" 不符合mac地址格式", w).Show()
						}
						if !mac_regexp.MatchString(dest_mac_entry.Text) {
				...
				dest_port_entry.Text = DestPort_filter
				// we can also append items
				form.Append("源Port", source_mac_entry)
				form.Append("目的Port", dest_mac_entry)
				form.Append("源IP", source_ip_entry)
				form.Append("目的IP", dest_ip_entry)
				form.Append("源Port", source_port_entry)
				form.Append("目的Port", dest_port_entry)
